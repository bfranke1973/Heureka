...
static bool CompareNumbers(const char* F1P, const char* F2P, const char* F1End, const char* F2End,
...
int diff_file(const char* path_a, const char* path_b, bool parse_fp, double absolute_tolerance,
              double relative_tolerance, bool ignore_whitespace) {
  ...
  while (1) {
    ...
    // they don't match.
    if (CompareNumbers(F1NumStart, F2NumStart, F1NumEnd, F2NumEnd, absolute_tolerance,
                       relative_tolerance)) {
      free(data_a);
      free(data_b);
      return 1;
  ...
  if (!F1AtEnd && !F2AtEnd) {
    fprintf(stderr, "%s: Comparison failed, textual difference between '%c' and '%c'\n", g_program,
            F1P[0], F2P[0]);
    free(data_a);
    free(data_b);
    return 1;
  ...
  if (!F1AtEnd || !F2AtEnd) {
    fprintf(stderr, "%s: Comparison failed, unexpected end of one of the files\n", g_program);
    free(data_a);
    free(data_b);
    return 1;
  }
...
void usage() {
  ...
  fprintf(stderr,
          "Search two text files for differences.\n"
...
int main(int argc, char* const argv[]) {
  ...
  return diff_file(argv[i], argv[i + 1], parse_fp, absolute_tolerance, relative_tolerance,
                   ignore_whitespace);
}
