...
class ProcWorker:
    ...
    def init_signals(self):
        logger.debug("Entering init_signals")
    ...
    def main_func(self, *args):
        logger.debug("Entering main_func")
    ...
    def run(self):
        ...
        try:
            ...
            logger.error(f"Exception Shutdown: {exc}", exc_info=True)
...
class TimerProcWorker(ProcWorker):
    ...
    def main_loop(self):
        logger.debug("Entering TimerProcWorker.main_loop")
        next_time = time.time() + self.INTERVAL_SECS
        while not self.shutdown_event.is_set():
            sleep_secs = _sleep_secs(self.MAX_SLEEP_SECS, next_time)
            time.sleep(sleep_secs)
            if time.time() > next_time:
                logger.debug("TimerProcWorker.main_loop : calling main_func")
                self.main_func()
                next_time = time.time() + self.INTERVAL_SECS
...
class QueueProcWorker(ProcWorker):
    def init_args(self, args):
        logger.debug(f"Entering QueueProcWorker.init_args : {args}")
        (self.work_q,) = args

    def main_loop(self):
        logger.debug("Entering QueueProcWorker.main_loop")
        while not self.shutdown_event.is_set():
            item = self.work_q.safe_get()
            if not item:
                continue
            logger.debug(f"QueueProcWorker.main_loop received '{item}' message")
            if item == "END":
                break
            else:
                self.main_func(item)
...
class Proc:
    ...
    def __init__(self, name, worker_class, shutdown_event, event_q, *args):
        ...
        logger.debug(f"Proc.__init__ starting : {name}")
        self.proc.start()
        started = self.startup_event.wait(timeout=Proc.STARTUP_WAIT_SECS)
        logger.debug(f"Proc.__init__ starting : {name} got {started}")
        if not started:
            self.terminate()
            raise RuntimeError(
    ...
    def full_stop(self, wait_time=SHUTDOWN_WAIT_SECS):
        logger.debug(f"Proc.full_stop stoping : {self.name}")
    ...
    def terminate(self):
        logger.debug(f"Proc.terminate terminating : {self.name}")
        NUM_TRIES = 3
        tries = NUM_TRIES
...
class MainContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type:
            logger.error(f"Exception: {exc_val}", exc_info=(exc_type, exc_val, exc_tb))
    ...
    def stop_procs(self):
        ...
        while self.procs:
            ...
            if proc.proc.is_alive():
                ...
                if exitcode:
                    logger.error(f"Process {proc.name} ended with exitcode {exitcode}")
                    num_failed += 1
                else:
                    logger.debug(f"Process {proc.name} stopped successfully")
